 ...
 */
package io.smusz.maciej.database;

import io.smusz.maciej.crypto.CryptoException;
import io.smusz.maciej.crypto.DESDecryptionService;
import io.smusz.maciej.crypto.EncryptionService;
 ...
 * of the io.smusz.maciej.database need to be loaded slightly differently so this class takes
 ...
 * 
 *   DB_VERSION = The structural version of the io.smusz.maciej.database
 *   SALT = The salt used to mix with the user password to create the key
 *   DB_HEADER = Was used to store the structural version of the io.smusz.maciej.database (pre version 2)
 *   DB_OPTIONS = Options relating to the io.smusz.maciej.database
 *   ACCOUNTS = The account information
 *   
 *   From version 2 the db version is stored unencrypted at the start of the file.
 *   This allows for cryptographic changes in the io.smusz.maciej.database structure. Before this
 *   we had to know how to unencrypt the io.smusz.maciej.database before we could find out the version number.
 */
public class PasswordDatabasePersistence {
    ...
    /**
     * Used when we want to create a new io.smusz.maciej.database with the given password
     * @param password
...
//    	}
        ...
        // Check the io.smusz.maciej.database is a minimum length
        if (fullDatabase.length < EncryptionService.SALT_LENGTH) {
            throw new ProblemReadingDatabaseFile("This file doesn't appear to be a UPM password io.smusz.maciej.database");
        }
        ...
        // Ensure this is a real UPM io.smusz.maciej.database by checking for the existence of
        ...
        if (Arrays.equals(header, FILE_HEADER.getBytes())) {
            ...
            // Get the io.smusz.maciej.database version
            ...
            // This might be an old io.smusz.maciej.database (pre version 2).
    ...
    public PasswordDatabase load(File databaseFile, char[] password) throws IOException, ProblemReadingDatabaseFile, InvalidPasswordException, CryptoException {
        ...
        if (Arrays.equals(header, FILE_HEADER.getBytes())) {
            ...
            // This might be an old io.smusz.maciej.database (pre version 2) so try loading it using the old io.smusz.maciej.database format
            ...
            // At this point we'll check to see what version the io.smusz.maciej.database is and load it accordingly
            if (dh.getVersion().equals("1.1.0")) {
                // Version 1.1.0 introduced a revision number & io.smusz.maciej.database options so read that in now
                revision = new Revision(is);
                dbOptions = new DatabaseOptions(is);
                ...
                throw new ProblemReadingDatabaseFile("Don't know how to handle io.smusz.maciej.database version [" + dh.getVersion() + "]");
            }
    ...
    public void save(PasswordDatabase database) throws IOException, CryptoException {
        ...
        // Flatpack the io.smusz.maciej.database revision and options
        ...
        //Now encrypt the io.smusz.maciej.database data
        byte[] encryptedData = encryptionService.encrypt(dataToEncrypt);
        
        //Write the salt and the encrypted data out to the io.smusz.maciej.database file
