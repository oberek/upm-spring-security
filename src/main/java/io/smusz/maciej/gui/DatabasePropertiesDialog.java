...
public class DatabasePropertiesDialog extends EscapeDialog {
    ...
    private void saveDatabaseOptions(JFrame parentFrame, String remoteLocation, String authEntry, PasswordDatabase database) {
        ...
        // the flag to indicate that the io.smusz.maciej.database needs to be saved
        ...
        // If the url/remoteLocation is not empty then we need to validate the URL and upload the io.smusz.maciej.database
        if (!remoteLocation.equals("")) {
            ...
            if (url != null) {
                ...
                if (Transport.isASupportedProtocol(url.getProtocol())) {
                    ...
                    // If the remote location has changed then upload the io.smusz.maciej.database
                    if (!database.getDbOptions().getRemoteLocation().equals(remoteLocation)) {
                        try {
        ...
        // Attempt to save the io.smusz.maciej.database and then close the window
        if (canCloseWindow) {
            try {
