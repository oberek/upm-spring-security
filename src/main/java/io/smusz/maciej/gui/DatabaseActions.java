...
import io.smusz.maciej.database.ImportException;
import io.smusz.maciej.database.PasswordDatabase;
import io.smusz.maciej.database.PasswordDatabasePersistence;
import io.smusz.maciej.database.ProblemReadingDatabaseFile;
import io.smusz.maciej.gui.MainWindow.ChangeDatabaseAction;
import io.smusz.maciej.transport.Transport;
import io.smusz.maciej.transport.TransportException;
import io.smusz.maciej.util.FileChangedCallback;
import io.smusz.maciej.util.FileMonitor;
import io.smusz.maciej.util.Preferences;
import io.smusz.maciej.util.Translator;
import io.smusz.maciej.util.Util;
...
public class DatabaseActions {
    ...
    /**
     * This method asks the user for the name of a new io.smusz.maciej.database and then creates
    ...
    public void newDatabase() throws IOException, CryptoException {
        ...
        do {
            ...
            //Get a new master password for this io.smusz.maciej.database from the user
        ...
        // user if they'd like to open this io.smusz.maciej.database on startup.
    ...
    public void changeMasterPassword() throws IOException, ProblemReadingDatabaseFile, CryptoException, PasswordDatabaseException, TransportException {
        ...
        if (getLatestVersionOfDatabase()) {
            ...
            if (passwordCorrect == true) {
                    ...
                    //If the user clicked OK and the passwords match then change the io.smusz.maciej.database password
    ...
    public void deleteAccount() throws IOException, CryptoException, TransportException, ProblemReadingDatabaseFile, PasswordDatabaseException {
        ...
        if (getLatestVersionOfDatabase()) {
            ...
            if (buttonSelected == JOptionPane.OK_OPTION) {
                //Remove the account from the listview, accountNames arraylist & the io.smusz.maciej.database
    ...
    public void addAccount() throws IOException, CryptoException, TransportException, ProblemReadingDatabaseFile, PasswordDatabaseException {
        ...
        if (getLatestVersionOfDatabase()) {
            ...
            //If the user press OK then save the new account to the io.smusz.maciej.database
    ...
    private boolean getLatestVersionOfDatabase() throws TransportException, ProblemReadingDatabaseFile, IOException, CryptoException, PasswordDatabaseException {
        ...
        // Ensure we're working with the latest version of the io.smusz.maciej.database
    ...
    public void editAccount(String accountName) throws TransportException,
        ...
        if (getLatestVersionOfDatabase()) {
            ...
            //If the ok button was clicked then save the account to the io.smusz.maciej.database and update the
    ...
    public void openDatabaseFromURL() throws TransportException, IOException, ProblemReadingDatabaseFile, CryptoException {
        ...
        // Ask the user for the remote io.smusz.maciej.database location
        ...
        if (openDBDialog.getOkClicked()) {
            // Get the remote io.smusz.maciej.database options
            ...
            // Ask the user for a location to save the io.smusz.maciej.database file to
            File saveDatabaseTo = getSaveAsFile(Translator.translate("saveDatabaseAs"));

            if (saveDatabaseTo != null) {

                // Download the io.smusz.maciej.database
                ...
                // Save the downloaded io.smusz.maciej.database file to the new location
                Util.copyFile(downloadedDatabaseFile, saveDatabaseTo);

                // Now open the downloaded io.smusz.maciej.database
    ...
    public void reloadDatabase()
        ...
        try {
            ...
            // The password for the reloaded io.smusz.maciej.database is different to that of
            // the open io.smusz.maciej.database
    ...
    public boolean syncWithRemoteDatabase() throws TransportException, ProblemReadingDatabaseFile, IOException, CryptoException, PasswordDatabaseException {
        ...
        try {
            ...
            // Download the io.smusz.maciej.database that's already at the remote location
            Transport transport = Transport.getTransportForURL(new URL(remoteLocation));
            ...
            // Attempt to decrypt the io.smusz.maciej.database using the password the user entered
            ...
            try {
                ...
                // The password for the downloaded io.smusz.maciej.database is different to that of the open io.smusz.maciej.database
                // (most likely the user changed the local io.smusz.maciej.database's master password)
                boolean okClicked = false;
                do {
            ...
            /* If the local io.smusz.maciej.database revision > remote io.smusz.maciej.database version => upload local io.smusz.maciej.database
               If the local io.smusz.maciej.database revision < remote io.smusz.maciej.database version => replace local io.smusz.maciej.database with remote io.smusz.maciej.database
               If the local io.smusz.maciej.database revision = remote io.smusz.maciej.database version => do nothing */
            if (successfullyDecryptedDb) {
                ...
                if (syncSuccessful) {
                    ...
                    // Create a thread that will mark the io.smusz.maciej.database dirty after
                    // a short period. Without this the io.smusz.maciej.database would remain
                    ...
                    // with a conflicting version of the io.smusz.maciej.database.
                        ...
                        public void run() {
                            while (runSetDBDirtyThread) {
                                ...
                                if (currentTime - dirtyThreadStartTime > 5 * 60 * 1000) {
                                    LOG.info("SetDBDirtyThread setting io.smusz.maciej.database dirty");
                                    setLocalDatabaseDirty(true);
                                    runSetDBDirtyThread = false;
                                }
                            }
        ...
        if (getLatestVersionOfDatabase()) {
            ...
            if (returnVal == JFileChooser.APPROVE_OPTION) {
                ...
                try {
                    ...
                    // Add each account to the open io.smusz.maciej.database. If the account
    ...
    private File getSaveAsFile(String title) {
        ...
        do {
            ...
            //Warn the user if the io.smusz.maciej.database file already exists
    ...
    private class AutoLockDatabaseListener implements WindowFocusListener {
         ...
         * already registered then start a timer to close the io.smusz.maciej.database after the
            ...
            if (database != null && closeDBTimer == null){
                ...
                closeDBTimer.addActionListener(new ActionListener() {
                    public void actionPerformed(ActionEvent e) {
                        LOG.debug("Closing io.smusz.maciej.database due to inactivity");
